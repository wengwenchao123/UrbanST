...
def get_adjacency_matrix(distance_df_filename: str, num_of_vertices: int, id_filename: str = None) -> tuple:
    ...
    if "pkl" in distance_df_filename:
        f =open(distance_df_filename,'rb')
        adj_mx = pickle.load(f)[2]
        return adj_mx, None
    else:
...
def generate_adj_pems04():
    distance_df_filename, num_of_vertices = "adj_data/adj_mx_taxi.pkl", 266
    ...
    with open("adj_NYC-Taxi16.pkl", "wb") as f:
        pickle.dump(adj_mx, f)
    with open("adj_NYC-Taxi_distance.pkl", "wb") as f:
