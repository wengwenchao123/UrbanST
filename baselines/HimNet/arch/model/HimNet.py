...
class HimNet(nn.Module):
    def __init__(
        ...
        self.time_in_day_size = time_in_day_size
        self.day_in_week_size = day_in_week_size
        ...
        self.tod_embedding = nn.Embedding(self.time_in_day_size, tod_embedding_dim)
        self.dow_embedding = nn.Embedding(self.day_in_week_size, dow_embedding_dim)
    ...
    def forward(self, x, y, batches_seen=None):
        y_cov = y[..., -(self.input_dim-self.output_dim):]
        labels = y[...,:self.output_dim]
        tod = x[:, -1, 0, -2]
        dow = x[:, -1, 0, -1]
        tod_emb = self.tod_embedding(
            (tod * self.time_in_day_size).long()
        )  # (batch_size, tod_embedding_dim)
        
        dow_emb = self.dow_embedding((dow*self.day_in_week_size).long())  # (batch_size, dow_embedding_dim)
