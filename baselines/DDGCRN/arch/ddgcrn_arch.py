...
class DDGCRN(nn.Module):
    def __init__(self, num_nodes,input_dim,rnn_units,output_dim,horizon,num_layers,use_day,use_week,embed_dim,
                 cheb_k,time_of_day_size,day_of_week_size):
        ...
        self.node_embeddings1 = nn.Parameter(torch.randn(self.num_node, embed_dim), requires_grad=True)
        # self.node_embeddings2 = nn.Parameter(torch.randn(self.num_node, embed_dim), requires_grad=True)
        self.T_i_D_emb = nn.Parameter(torch.randn(288, embed_dim))
        self.D_i_W_emb = nn.Parameter(torch.randn(7, embed_dim))

        self.encoder1 = DGCRM(num_nodes, input_dim, rnn_units, cheb_k,
                              embed_dim, num_layers)
        self.encoder2 = DGCRM(num_nodes, input_dim, rnn_units, cheb_k,
                              embed_dim, num_layers)
        #predictor
        self.end_conv1 = nn.Conv2d(1, horizon * self.output_dim, kernel_size=(1, self.hidden_dim), bias=True)
        self.end_conv2 = nn.Conv2d(1, horizon * self.input_dim, kernel_size=(1, self.hidden_dim), bias=True)
        self.end_conv3 = nn.Conv2d(1, horizon * self.output_dim, kernel_size=(1, self.hidden_dim), bias=True)
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, **kwargs):
        ...
        # init_state = self.encoder.init_hidden(history_data.shape[0])   #[2,64,307,64] 前面是2是因为有两层GRU
        ...
        if self.use_D:
            ...
            T_i_D_emb = self.T_i_D_emb[(t_i_d_data * self.time_of_day_size).type(torch.LongTensor)]
        ...
        if self.use_W:
            ...
            D_i_W_emb = self.D_i_W_emb[(d_i_w_data * self.day_of_week_size).type(torch.LongTensor)]
        ...
        init_state1 = self.encoder1.init_hidden(history_data.shape[0])   #[2,64,307,64] 前面是2是因为有两层GRU
        output, _ = self.encoder1(history_data, init_state1, node_embeddings)      #B, T, N, hidden
        # output = output[:, -1:, :, :]                                   #B, 1, N, hidden
        output = self.dropout1(output[:, -1:, :, :])

        #CNN based predictor
        output1 = self.end_conv1(output).reshape(output.shape[0],-1,self.num_node,self.output_dim)                         #B, T*C, N, 1

        source1 = self.end_conv2(output).reshape(output.shape[0],-1,self.num_node,self.input_dim)

        source2 = history_data -source1
        ...
        output2 = self.end_conv3(output2).reshape(output.shape[0],-1,self.num_node,self.output_dim)
