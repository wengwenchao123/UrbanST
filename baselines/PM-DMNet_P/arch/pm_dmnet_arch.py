...
class PM_DMNet_P(nn.Module):
    def __init__(self, num_nodes,input_dim,rnn_units,output_dim,horizon,num_layers,use_day,use_week,embed_dim,cheb_k,time_dim,
                 type,teacher_forcing,teacher_decay_step,time_of_day_size,day_of_week_size):
        super(PM_DMNet_P, self).__init__()
        ...
        # self.node_embeddings2 = nn.Parameter(torch.randn(self.num_node, time_dim), requires_grad=True)
        self.T_i_D_emb = nn.Parameter(torch.empty(288, time_dim))
        self.D_i_W_emb = nn.Parameter(torch.empty(7, time_dim))
        self.S_i_W_emb = nn.Parameter(torch.empty(7*288, time_dim))
        self.encoder = PM_Encoder(num_nodes, input_dim, rnn_units, cheb_k,
                                  embed_dim, time_dim, num_layers)
        # self.decoder = PM_Decoder(num_nodes, input_dim, rnn_units, cheb_k,
        #                           embed_dim, time_dim, num_layers)
        #predictor
        self.proj = nn.Sequential(nn.Linear(self.hidden_dim, self.output_dim, bias=True))
        self.end_conv1 = nn.Conv2d(1, horizon * self.output_dim, kernel_size=(1, self.hidden_dim), bias=True)
        # self.end_conv2 = nn.Conv2d(12, 12, kernel_size=(1, 1), bias=True)
        if type == 'P':
            self.TA = TransformAttentionModel(self.hidden_dim, time_dim,embed_dim)
            self.decoder = Parallel_decoder(num_nodes,input_dim,rnn_units,output_dim,horizon,num_layers,use_day,use_week,embed_dim,cheb_k,time_dim)
        elif type =='R':
            self.decoder = Recurrent_decoder(num_nodes,input_dim,rnn_units,output_dim,horizon,num_layers,use_day,use_week,embed_dim,cheb_k,time_dim,
                                             teacher_forcing,teacher_decay_step)
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, **kwargs):
        #history_data: B, T_1, N, D
        #target: B, T_2, N, D


        t_i_d_data1 = history_data[..., 0, -2]
        t_i_d_data2 = future_data[..., 0, -2]
        # T_i_D_emb = self.T_i_D_emb[(t_i_d_data[:, -1, :] * 288).type(torch.LongTensor)]
        T_i_D_emb1 = self.T_i_D_emb[(t_i_d_data1 * self.time_of_day_size).type(torch.LongTensor)]
        T_i_D_emb2 = self.T_i_D_emb[(t_i_d_data2 * self.time_of_day_size).type(torch.LongTensor)]

        d_i_w_data1 = history_data[..., 0, -1]
        d_i_w_data2 = future_data[..., 0, -1]
        # D_i_W_emb = self.D_i_W_emb[(d_i_w_data[:, -1, :]).type(torch.LongTensor)]
        D_i_W_emb1 = self.D_i_W_emb[(d_i_w_data1 * self.day_of_week_size).type(torch.LongTensor)]
        D_i_W_emb2 = self.D_i_W_emb[(d_i_w_data2 * self.day_of_week_size).type(torch.LongTensor)]
        ...
        # d_i_w_data1 = history_data[..., 0, -1]
        # d_i_w_data2 = future_data[..., 0, -1]
        # node_embedding1 = self.S_i_W_emb[((t_i_d_data1+ d_i_w_data1 * self.day_of_week_size)* self.time_of_day_size).type(torch.LongTensor)]
        # node_embedding2 = self.S_i_W_emb[((t_i_d_data2+ d_i_w_data2 * self.day_of_week_size)* self.time_of_day_size).type(torch.LongTensor)]

        en_node_embeddings=[node_embedding1,self.node_embeddings]

        # history_data = history_data[..., :self.input_dim].unsqueeze(-1)
        history_data = history_data[..., :self.input_dim]

        init_state = self.encoder.init_hidden(history_data.shape[0])  # [2,64,307,64]
        state, h_n = self.encoder(history_data, init_state, en_node_embeddings)  # B, T, N, hidden

        output =self.decoder(history_data, future_data,h_n,node_embedding1, node_embedding2,self.node_embeddings,batch_seen)
        ...
        # # return output
        #
        # go = torch.zeros((history_data.shape[0]*self.horizon, self.num_node, self.output_dim), device=history_data.device)
        #
        #
        # state, ht_list = self.decoder(go, [de_input], [node_embedding2, self.node_embeddings])
        # go = self.proj(self.dropout(state))
        # output = go.reshape(history_data.shape[0],self.horizon,self.num_node,self.output_dim)
...
class Parallel_decoder(nn.Module):
    def __init__(self,num_nodes,input_dim,rnn_units,output_dim,horizon,num_layers,use_day,use_week,embed_dim,cheb_k,time_dim,):
        ...
        self.TA= TransformAttentionModel(rnn_units, time_dim,embed_dim)
        self.num_node = num_nodes
        self.input_dim = input_dim
        self.hidden_dim = rnn_units
        self.output_dim = output_dim
        ...
        # self.end_conv = nn.Conv2d(horizon, horizon * self.output_dim, kernel_size=(1, self.hidden_dim), bias=True)
    def forward(self, history_data, future_data, h_n,node_embedding1, node_embedding2,node_embeddings,batches_seen):
        h_n = h_n[0]
        h_n = h_n.unsqueeze(1)
        de_input = self.TA(h_n,node_embedding1[:,-1,:].unsqueeze(1),node_embedding2).flatten(0, 1)
        # de_input = h_n.expand(-1,self.horizon,-1,-1).flatten(0, 1)
        ...
        state = state.reshape(history_data.shape[0],self.horizon,self.num_node,self.hidden_dim)
        output = torch.matmul(self.dropout(state), self.weights)+self.bias.unsqueeze(dim=-2)

        # output = self.end_conv(self.dropout(state)).reshape(history_data.shape[0],self.horizon,self.num_node,self.output_dim)
        #
        return output
...
class Recurrent_decoder(nn.Module):
    def __init__(self,num_nodes,input_dim,rnn_units,output_dim,horizon,num_layers,use_day,use_week,embed_dim,cheb_k,time_dim,
        ...
        # self.cl_decay_steps = lr_decay_step
        self.num_node = num_nodes
        self.input_dim = input_dim
        self.hidden_dim = rnn_units
        self.output_dim = output_dim
        ...
        self.teacher_decay_step = teacher_decay_step
        self.decoder = PM_Decoder(num_nodes, input_dim, rnn_units, cheb_k,
                                  embed_dim, time_dim, num_layers)
        self.proj = nn.Sequential(nn.Linear(self.hidden_dim, self.output_dim, bias=True))
    def forward(self, history_data, future_data, h_n, node_embedding1,node_embedding2,node_embeddings, batches_seen):
        ht_list= h_n
        go = torch.zeros((history_data.shape[0], self.num_node, self.output_dim), device=history_data.device)
        out = []
        for t in range(self.horizon):
            ...
            if self.training and self.teacher_forcing:     #这里的课程学习用了给予一定概率用真实值代替预测值来学习的教师-学生学习法（名字忘了，大概跟着有关）
                ...
                if c < self._compute_sampling_threshold(batches_seen):  #如果满足条件，则用真实值代替预测值训练
                    # go = future_data[:, t, :, :self.input_dim].unsqueeze(-1)
                    go = future_data[:, t, :, :self.input_dim]
