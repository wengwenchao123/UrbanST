...
class DGCRN(nn.Module):
    ...
    """
    def __init__(self, gcn_depth, num_nodes, predefined_A=None, dropout=0.3, subgraph_size=20, node_dim=40, middle_dim=2, seq_length=12, in_dim=2, out_dim=1,list_weight=[0.05, 0.95, 0.95], tanhalpha=3, cl_decay_steps=4000, rnn_size=64, hyperGNN_dim=16):
        ...
        for i in range(task_level):
            ...
            if self.training and self.use_curriculum_learning:
                ...
                if c < self._compute_sampling_threshold(batch_seen):
                    decoder_input = ycl[:, :self.output_dim, :, i]
