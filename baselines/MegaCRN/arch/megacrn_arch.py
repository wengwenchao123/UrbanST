...
class MegaCRN(nn.Module):
    ...
    def __init__(self, num_nodes, input_dim, output_dim, horizon, rnn_units, num_layers=1, cheb_k=3,
        ...
        # deocoder
        self.decoder_dim = self.rnn_units + self.mem_dim   #input_dim = output_dim+ycov_dim
        self.decoder = ADCRNN_Decoder(self.num_nodes, self.input_dim, self.decoder_dim, self.cheb_k, self.num_layers)
    ...
    # def forward(self, x, y_cov, labels=None, batches_seen=None):
        ...
        y_cov = future_data[..., self.output_dim:]
        labels = future_data[..., :self.output_dim]
