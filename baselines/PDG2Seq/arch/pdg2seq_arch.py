...
from .pdg2seqcell import PDG2SeqCell
import numpy as np
...
class PDG2Seq(nn.Module):
    def __init__(self, num_nodes,input_dim,rnn_units,output_dim,horizon,num_layers,use_day,use_week,time_dim,embed_dim,
                 cheb_k,lr_decay_step,time_of_day_size,day_of_week_size):
        ...
        self.node_embeddings1 = nn.Parameter(torch.empty(self.num_node, embed_dim))
        self.T_i_D_emb1 = nn.Parameter(torch.empty(288, time_dim))
        self.D_i_W_emb1 = nn.Parameter(torch.empty(7, time_dim))
        self.T_i_D_emb2 = nn.Parameter(torch.empty(288, time_dim))
        self.D_i_W_emb2 = nn.Parameter(torch.empty(7, time_dim))

        self.encoder = PDG2Seq_Encoder(num_nodes, input_dim, rnn_units, cheb_k,
                                       embed_dim, time_dim, num_layers)
        self.decoder = PDG2Seq_Dncoder(num_nodes, input_dim, rnn_units, cheb_k,
                                       embed_dim, time_dim, num_layers)
        #predictor
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, **kwargs):
        ...
        T_i_D_emb1_en = self.T_i_D_emb1[(t_i_d_data1 * self.time_of_day_size).type(torch.LongTensor)]
        T_i_D_emb2_en = self.T_i_D_emb2[(t_i_d_data1 * self.time_of_day_size).type(torch.LongTensor)]

        T_i_D_emb1_de = self.T_i_D_emb1[(t_i_d_data2 * self.time_of_day_size).type(torch.LongTensor)]
        T_i_D_emb2_de = self.T_i_D_emb2[(t_i_d_data2 * self.time_of_day_size).type(torch.LongTensor)]
        if self.use_W:
            d_i_w_data1 = source[..., 0,-1]
            d_i_w_data2 = traget[..., 0,-1]
            # D_i_W_emb = self.D_i_W_emb[(d_i_w_data[:, -1, :]).type(torch.LongTensor)]
            D_i_W_emb1_en = self.D_i_W_emb1[(d_i_w_data1 * self.day_of_week_size).type(torch.LongTensor)]
            D_i_W_emb2_en = self.D_i_W_emb2[(d_i_w_data1 * self.day_of_week_size).type(torch.LongTensor)]

            D_i_W_emb1_de = self.D_i_W_emb1[(d_i_w_data2 * self.day_of_week_size).type(torch.LongTensor)]
            D_i_W_emb2_de = self.D_i_W_emb2[(d_i_w_data2 * self.day_of_week_size).type(torch.LongTensor)]
        ...
        go = torch.zeros((source.shape[0], self.num_node, self.input_dim), device=source.device)
        out = []
        for t in range(self.horizon):
            state, ht_list = self.decoder(go, ht_list, [node_embedding_de1[:, t, :], node_embedding_de2[:, t, :], self.node_embeddings1])
            go = self.proj(state)
            out.append(go)
            go = torch.cat([go,traget[:, t, :, self.output_dim:self.input_dim]],dim=-1)
            # out.append(go)
            if self.training:     #这里的课程学习用了给予一定概率用真实值代替预测值来学习的教师-学生学习法（名字忘了，大概跟着有关）
                c = np.random.uniform(0, 1)
                if c < self._compute_sampling_threshold(batch_seen):  #如果满足条件，则用真实值代替预测值训练
