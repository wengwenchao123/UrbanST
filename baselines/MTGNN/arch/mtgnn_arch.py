...
class MTGNN(nn.Module):
    ...
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, predefined_A=None, static_feat=None, dropout=0.3, subgraph_size=20, node_dim=40, dilation_exponential=1, conv_channels=32, residual_channels=32, skip_channels=64, end_channels=128, seq_length=12, in_dim=2, out_dim=1, out_length=12, layers=3, propalpha=0.05, tanhalpha=3, layer_norm_affline=True):
        ...
        self.end_conv_2 = nn.Conv2d(
            in_channels=end_channels, out_channels=out_dim*out_length, kernel_size=(1, 1), bias=True)
        if self.seq_length > self.receptive_field:
    ...
    def forward(self, history_data: torch.Tensor, idx: int = None, **kwargs) -> torch.Tensor:
        ...
        # print(x.shape)
        x = self.end_conv_2(x).reshape(-1,self.out_length,self.num_nodes,self.out_dim)
        # print(x.shape)
