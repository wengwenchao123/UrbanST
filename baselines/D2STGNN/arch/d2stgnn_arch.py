...
class D2STGNN(nn.Module):
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, **kwargs) -> torch.Tensor:
        ...
        forecast=forecast.view(forecast.shape[0], forecast.shape[1], -1, self._out_feat)

        # reshape
        forecast = forecast.transpose(1, 2)

        #不知道为什么,这样子直接一步到位效果很差,一定要先把时间和节点两个维度调换后再view,然后再调换回来效果才好
        # forecast = self.out_fc_2(
        #     F.relu(self.out_fc_1(F.relu(forecast_hidden))))
        # forecast = forecast.contiguous()
        # forecast=forecast.view(forecast.shape[0], -1, forecast.shape[2],self._out_feat)
        #
        # # reshape
        # # forecast = forecast.transpose(1, 2)
