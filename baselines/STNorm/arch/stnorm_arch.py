...
class STNorm(nn.Module):
    ...
    def __init__(self, num_nodes, tnorm_bool, snorm_bool, in_dim, out_dim, out_length, channels, kernel_size, blocks, layers):
        ...
        self.end_conv_2 = nn.Conv2d(in_channels=channels,
                                    out_channels=out_length*out_dim,
                                    kernel_size=(1, 1),
                                    bias=True)
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, **kwargs) -> torch.Tensor:
        ...
        out = self.end_conv_2(rep).reshape(rep.shape[0],self.out_length,-1,self.out_dim)
        return out
